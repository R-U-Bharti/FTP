...
export interface StartServerOptions {
  /** Port to try first (scans upward if busy). Defaults to $PORT or 3001. */
  port?: number;
  /** Directory to expose for sharing. Defaults to $SHARED_DIR or ~/LocalDrop. */
  sharedDir?: string;
  /**
   * Open the served URL in the system browser once listening.
   * Defaults to true (CLI/pkg build). The Electron desktop app passes false —
   * it renders the UI in its own native window instead.
   */
  openBrowser?: boolean;
}

export interface RunningServer {
  /** The port the HTTP/Socket.IO server is actually listening on. */
  port: number;
  /** Convenience URL for the local UI, e.g. http://localhost:3001. */
  url: string;
  /** Device id advertised on the network. */
  deviceId: string;
  /** Root directory being shared. */
  sharedDir: string;
  /** Stop discovery, sockets, and the HTTP server. */
  close(): Promise<void>;
}

/**
 * Boot the full LocalDrop server (Express + Socket.IO + UDP discovery) and
 * resolve once it is listening. Shared by the CLI entry point and the Electron
 * desktop shell so there is a single source of truth for startup.
 */
export async function startServer(options: StartServerOptions = {}): Promise<RunningServer> {
  const desiredPort =
    options.port ?? parseInt(process.env['PORT'] || '3001', 10);
  const sharedDirOption = options.sharedDir ?? (process.env['SHARED_DIR'] || undefined);
  const shouldOpenBrowser = options.openBrowser ?? true;

  const PORT = await findFreePort(desiredPort);

  // ── Initialize services ──
  const fsService = new FileSystemService(sharedDirOption);
  ...
  if (webServed) {
    if (shouldOpenBrowser) {
      ...
      console.log(`  🌐 Web UI ready at ${url}`);
    }
  } else {
  ...
  return {
    ...
    sharedDir: fsService.getRootDir(),
    close,
  };
}
